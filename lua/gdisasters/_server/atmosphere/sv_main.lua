...
function Wind()
	...
	for k, v in pairs(player.GetAll()) do
		...
		if isOutdoor==false or IsSomethingBlockingWind(v) then local_wind = 0 end 
	...
	if windphysics_enabled and windspeed >= 1 and CurTime() >= GLOBAL_SYSTEM["Atmosphere"]["Wind"]["NextThink"] and GetConVar("gdisasters_heat_system_enabled"):GetInt() <= 0 then
