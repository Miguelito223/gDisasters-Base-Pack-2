...
local chaseMusic = Sound("streams/nextbot/gd_npc_xshitter/xshitter.mp3")

local IsValid = IsValid

if SERVER then -- SERVER --

local gd_npc_xshitter_acquire_distance =
	CreateConVar("gd_npc_xshitter_acquire_distance", 9500, FCVAR_NONE,
	"The maximum distance at which xshitter will chase a target.")

local gd_npc_xshitter_spawn_protect =
	CreateConVar("gd_npc_xshitter_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, xshitter will not target players or hide within 200 units of \z
	a spawn point.")

local gd_npc_xshitter_attack_distance =
	CreateConVar("gd_npc_xshitter_attack_distance", 60, FCVAR_NONE,
	"The reach of xshitter's attack.")

local gd_npc_xshitter_attack_interval =
	CreateConVar("gd_npc_xshitter_attack_interval", 0.1, FCVAR_NONE,
	"The delay between xshitter's attacks.")

local gd_npc_xshitter_attack_force =
	CreateConVar("gd_npc_xshitter_attack_force", 9000, FCVAR_NONE,
	"The physical force of xshitter's attack. Higher values throw things \z
	farther.")

local gd_npc_xshitter_smash_props =
	CreateConVar("gd_npc_xshitter_smash_props", 1, FCVAR_NONE,
	"If set to 1, xshitter will punch through any props placed in their way.")

local gd_npc_xshitter_allow_jump =
	CreateConVar("gd_npc_xshitter_allow_jump", 1, FCVAR_NONE,
	"If set to 1, xshitter will be able to jump.")

local gd_npc_xshitter_hiding_scan_interval =
	CreateConVar("gd_npc_xshitter_hiding_scan_interval", 3, FCVAR_NONE,
	"xshitter will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant xshitters are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local gd_npc_xshitter_hiding_repath_interval =
	CreateConVar("gd_npc_xshitter_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to xshitter's hiding spot will be redetermined every X seconds.")

local gd_npc_xshitter_chase_repath_interval =
	CreateConVar("gd_npc_xshitter_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of xshitter's target will be redetermined every \z
	X seconds.")

local gd_npc_xshitter_expensive_scan_interval =
	CreateConVar("gd_npc_xshitter_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local gd_npc_xshitter_force_download =
	CreateConVar("gd_npc_xshitter_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download xshitter resources \z
...
util.AddNetworkString("xshitter_nag")
util.AddNetworkString("xshitter_navgen")
...
local VECTOR_xshitter_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_xshitter_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("gd_npc_xshitter: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other xshitters, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "gd_npc_xshitter"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "xshitterMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "gd_npc_xshitter" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why xshitter isn't working.
	net.Start("xshitter_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("gd_npc_xshitter: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("gd_npc_xshitter: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("gd_npc_xshitter: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("gd_npc_xshitter: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("gd_npc_xshitter: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, xshitter in pairs(ents.FindByClass("gd_npc_xshitter")) do
		xshitter:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "xshitterNavGen", navEndGenerate)
	else
		print("gd_npc_xshitter: nav_generate failed to initialize")
...
concommand.Add("gd_npc_xshitter_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("gd_npc_xshitter: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. xshitter doesn't use
	...
	elseif isConsole then
		print("gd_npc_xshitter: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = gd_npc_xshitter_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `gd_npc_xshitter_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if gd_npc_xshitter_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = gd_npc_xshitter_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. xshitter means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not gd_npc_xshitter_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = gd_npc_xshitter_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = gd_npc_xshitter_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = gd_npc_xshitter_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = gd_npc_xshitter_attack_distance:GetInt()
		...
		local repathInterval = gd_npc_xshitter_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and gd_npc_xshitter_allow_jump:GetBool()
		...
		local hidingScanInterval = gd_npc_xshitter_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = gd_npc_xshitter_hiding_repath_interval:GetFloat()
...
local MAT_xshitter = Material("nextbot/gd_npc_xshitter/xshitter")
killicon.Add("gd_npc_xshitter", "nextbot/gd_npc_xshitter/xshitter", color_white)
language.Add("gd_npc_xshitter", "xshitter ")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("gd_npc_xshitter: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a xshitter.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local gd_npc_xshitter_music_volume =
	CreateConVar("gd_npc_xshitter_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by xshitter. (0-1, where 0 is muted)")

-- If another xshitter comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, xshitters do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_xshitter_PANIC_COUNT xshitters are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of xshitter.
local MUSIC_xshitter_PANIC_COUNT = 8

local MUSIC_xshitter_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_xshitter_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("gd_npc_xshitter") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("xshitterPanicMusicUpdate")
	...
	local userVolume = math.Clamp(gd_npc_xshitter_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "gd_npc_xshitter" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_xshitter_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear xshitter taunt us.
...
local function startTimer()
	if not timer.Exists("xshitterPanicMusicUpdate") then
		timer.Create("xshitterPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_xshitter)

	-- Get the normal vector from xshitter to the player's eyes, and then compute
	...
	-- xshitter should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("xshitterHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("xshitterHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("xshitter is studying this map.", "xshitterHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "xshitterHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "xshitterHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "xshitterHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "xshitterHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("xshitter_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "xshitterNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. xshitter doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("gd_npc_xshitter_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for xshitter to figure \z
		this map out.\n\z
...
net.Receive("xshitter_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! xshitter doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet xshitter-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! xshitter doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to xshitter.\n\z
			\n\z
			If you ARE the server host, you can run gd_npc_xshitter_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not xshitter-compatible!",
			"Ok", nil,
...
list.Set( "NPC", "gd_npc_xshitter", {
	Name = "xshitter", 
	Class = "gd_npc_xshitter", 
	Category = "Nextbot",
	AdminOnly = false,
})
