...
function ENT:Phase()
	...
	elseif t_elapsed >= 120 and t_elapsed < 200 then
		self.State = "tornado_form"
	else
		self.State = "dead"
	end
...
function ENT:StateProcessor()
	...
	elseif self.State == "heavy_raining" then 
		self:HeavyRaining()
	elseif self.State == "tornado_form" then
		self:TornadoForm()
...
function ENT:TornadoForm()
	...
	GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(450,450),["Direction"]=Vector(0,1,0)}, ["Pressure"]    = 90000, ["Temperature"] = math.random(10,13), ["Humidity"]    = math.random(82,85), ["BRadiation"]  = 0.1}}
	if recentTor == false then
	Tornadospawn()
	return end
