...
function ENT:Phase()
	...
	elseif t_elapsed >= 70 and t_elapsed < 140 then
		self.State = "heavy_raining" 
	elseif t_elapsed >= 140 and t_elapsed < 165 then
		self.State = "dead" 
	elseif t_elapsed >= 165 and t_elapsed < 175 then
		self.State = "moderate_raining" 
	elseif t_elapsed >= 175 and t_elapsed < 225 then
		self.State = "heavy_raining" 
	elseif t_elapsed >= 225 and t_elapsed < 245 then
		self.State = "light_raining" 
	else
		self.State = "remove"
	end
...
function ENT:Eye()
	GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(0,2),["Direction"]=Vector(0,1,0)}, ["Pressure"]    = 65000, ["Temperature"] = math.random(23,23), ["Humidity"]    = math.random(25,30), ["BRadiation"]  = 0.1}}
...
function ENT:LightRaining()
	GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(25,45),["Direction"]=Vector(0,1,0)}, ["Pressure"]    = 85000, ["Temperature"] = math.random(18.5,21), ["Humidity"]    = math.random(42,45), ["BRadiation"]  = 0.1}}
...
function ENT:ModerateRaining()
	...
	GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(50,65),["Direction"]=Vector(0,1,0)}, ["Pressure"]    = 75000, ["Temperature"] = math.random(16,19), ["Humidity"]    = math.random(82,85), ["BRadiation"]  = 0.1}}
...
function ENT:HeavyRaining()
	...
	GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(230,251),["Direction"]=Vector(0,1,0)}, ["Pressure"]    = 66000, ["Temperature"] = math.random(15,18), ["Humidity"]    = math.random(92,93), ["BRadiation"]  = 0.1}}
...
function ENT:OnRemove()
	...
	if (SERVER) then		
		...
		for k, v in pairs(ents.FindByClass("gd_d3_cat5flood")) do
		v:Remove()
	end
	end
...
function ENT:StormSurge()
	...
	if !self:IsValid() then return end
		local ent = ents.Create("gd_d3_cat5flood")
		ent:SetPos(pos)
		ent:Spawn()
		ent:Activate()
