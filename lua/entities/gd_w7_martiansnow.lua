...
function ENT:Initialize()		
	...
	if (CLIENT) then
		...
		LocalPlayer().Sounds["Snowstorm_IDLE"]         = createLoopedSound(LocalPlayer(), "streams/disasters/nature/sandstorm_loop.wav")
		LocalPlayer().Sounds["Snowstorm_muffled_IDLE"] = createLoopedSound(LocalPlayer(), "streams/disasters/nature/sandstorm_muffled_loop.wav")
	end
	if (SERVER) then

		GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(70,95),["Direction"]=Vector(0,-1,0)}, ["Pressure"]    = 96000, ["Temperature"] = -100, ["Humidity"]    = math.random(10,45), ["BRadiation"]  = 100}}
	...
	for k, v in pairs(player.GetAll()) do
		...
		if v.gDisasters.Area.IsOutdoor then
			...
			if HitChance(3) then
				...
				InflictDamage(v, self, "acid", math.random(4,1)*math.random(10,20))
				InflictDamage(v, self, "cold", math.random(4,1)*math.random(10,20))
...
function ENT:Think()
	if (CLIENT) then
		...
		if   LocalPlayer().Sounds["Snowstorm_muffled_IDLE"]!=nil then
			 LocalPlayer().Sounds["Snowstorm_muffled_IDLE"]:ChangeVolume(muffled_volume, 0)
		end
		
		if   LocalPlayer().Sounds["Snowstorm_IDLE"]!=nil then
			 LocalPlayer().Sounds["Snowstorm_IDLE"]:ChangeVolume(idle_volume, 0)
		end
...
function ENT:OnRemove()
	...
	if (CLIENT) then
		...
		if LocalPlayer().Sounds["Snowstorm_IDLE"]!=nil then 
			LocalPlayer().Sounds["Snowstorm_IDLE"]:Stop()
			LocalPlayer().Sounds["Snowstorm_IDLE"]=nil
		end
		
		if LocalPlayer().Sounds["Snowstorm_muffled_IDLE"]!=nil then 
			LocalPlayer().Sounds["Snowstorm_muffled_IDLE"]:Stop()
			LocalPlayer().Sounds["Snowstorm_muffled_IDLE"]=nil
		end
