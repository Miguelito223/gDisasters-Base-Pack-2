...
function ENT:Phase()
	...
	elseif t_elapsed >= 30 and t_elapsed < 70 then
		self.State = "moderate_raining"
	elseif t_elapsed >= 70 and t_elapsed < 120 then
		self.State = "heavy_raining"  
	elseif t_elapsed >= 120 and t_elapsed < 140 then
		self.State = "moderate_raining" 
	elseif t_elapsed >= 140 and t_elapsed < 160 then
		self.State = "heavy_raining" 
	elseif t_elapsed >= 160 and t_elapsed < 185 then
		self.State = "light_raining" 
	else
		self.State = "remove"
	end
...
function ENT:Eye()
	GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(0,2),["Direction"]=Vector(0,1,0)}, ["Pressure"]    = 81000, ["Temperature"] = math.random(23,23), ["Humidity"]    = math.random(25,30), ["BRadiation"]  = 0.1}}
...
function ENT:LightRaining()
	GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(15,20),["Direction"]=Vector(0,1,0)}, ["Pressure"]    = 85000, ["Temperature"] = math.random(18,21), ["Humidity"]    = math.random(42,45), ["BRadiation"]  = 0.1}}
...
function ENT:ModerateRaining()
	...
	GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(40,60),["Direction"]=Vector(0,1,0)}, ["Pressure"]    = 84000, ["Temperature"] = math.random(15,18), ["Humidity"]    = math.random(82,85), ["BRadiation"]  = 0.1}}
...
function ENT:HeavyRaining()
	...
	GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(154,177),["Direction"]=Vector(0,1,0)}, ["Pressure"]    = 82000, ["Temperature"] = math.random(15,17), ["Humidity"]    = math.random(92,93), ["BRadiation"]  = 0.1}}
	...
	for k, v in pairs(player.GetAll()) do
		...
		if v.gDisasters.Area.IsOutdoor then
			...
			net.Send(v)
				...
				net.Start("gd_screen_particles")
				net.WriteString("hud/warp_ripple3")
				net.WriteFloat(math.random(5,400))
				net.WriteFloat(math.random(0,100)/100)
				net.WriteFloat(math.random(0,1))
				net.WriteVector(Vector(0,math.random(0,200)/100,0))
				net.Send(v)	
