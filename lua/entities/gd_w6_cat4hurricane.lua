...
function ENT:Phase()
	...
	elseif t_elapsed >= 120 and t_elapsed < 135 then
		self.State = "dead" 
	elseif t_elapsed >= 135 and t_elapsed < 150 then
		self.State = "moderate_raining" 
	elseif t_elapsed >= 150 and t_elapsed < 175 then
		self.State = "heavy_raining" 
...
function ENT:Eye()
	GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(0,2),["Direction"]=Vector(0,1,0)}, ["Pressure"]    = 68000, ["Temperature"] = math.random(23,23), ["Humidity"]    = math.random(25,30), ["BRadiation"]  = 0.1}}
...
function ENT:LightRaining()
	GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(15,45),["Direction"]=Vector(0,1,0)}, ["Pressure"]    = 85000, ["Temperature"] = math.random(18,21), ["Humidity"]    = math.random(42,45), ["BRadiation"]  = 0.1}}
...
function ENT:HeavyRaining()
	...
	GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(209,251),["Direction"]=Vector(0,1,0)}, ["Pressure"]    = 70000, ["Temperature"] = math.random(15,18), ["Humidity"]    = math.random(92,93), ["BRadiation"]  = 0.1}}
	...
	for k, v in pairs(player.GetAll()) do
		...
		if v.gDisasters.Area.IsOutdoor then
				...
				net.Start("gd_screen_particles")
				net.WriteString("hud/warp_ripple3")
				net.WriteFloat(math.random(5,550))
				net.WriteFloat(math.random(0,100)/100)
				net.WriteFloat(math.random(0,1))
				net.WriteVector(Vector(0,math.random(0,200)/100,0))
				net.Send(v)	
...
function ENT:OnRemove()
	...
	if (SERVER) then		
		...
		for k, v in pairs(ents.FindByClass("gd_d3_cat4flood")) do
		v:Remove()
	end
	end
...
function ENT:StormSurge()
	...
	if !self:IsValid() then return end
		local ent = ents.Create("gd_d3_cat4flood")
		ent:SetPos(pos)
		ent:Spawn()
		ent:Activate()
