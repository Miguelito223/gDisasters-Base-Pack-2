...
local chaseMusic = Sound("streams/nextbot/gd_npc_rainbow/rainbow.mp3")

local IsValid = IsValid

if SERVER then -- SERVER --

local gd_npc_rainbow_acquire_distance =
	CreateConVar("gd_npc_rainbow_acquire_distance", 9500, FCVAR_NONE,
	"The maximum distance at which rainbow will chase a target.")

local gd_npc_rainbow_spawn_protect =
	CreateConVar("gd_npc_rainbow_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, rainbow will not target players or hide within 200 units of \z
	a spawn point.")

local gd_npc_rainbow_attack_distance =
	CreateConVar("gd_npc_rainbow_attack_distance", 60, FCVAR_NONE,
	"The reach of rainbow's attack.")

local gd_npc_rainbow_attack_interval =
	CreateConVar("gd_npc_rainbow_attack_interval", 0.1, FCVAR_NONE,
	"The delay between rainbow's attacks.")

local gd_npc_rainbow_attack_force =
	CreateConVar("gd_npc_rainbow_attack_force", 9000, FCVAR_NONE,
	"The physical force of rainbow's attack. Higher values throw things \z
	farther.")

local gd_npc_rainbow_smash_props =
	CreateConVar("gd_npc_rainbow_smash_props", 1, FCVAR_NONE,
	"If set to 1, rainbow will punch through any props placed in their way.")

local gd_npc_rainbow_allow_jump =
	CreateConVar("gd_npc_rainbow_allow_jump", 1, FCVAR_NONE,
	"If set to 1, rainbow will be able to jump.")

local gd_npc_rainbow_hiding_scan_interval =
	CreateConVar("gd_npc_rainbow_hiding_scan_interval", 3, FCVAR_NONE,
...
local gd_npc_rainbow_hiding_repath_interval =
	CreateConVar("gd_npc_rainbow_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to rainbow's hiding spot will be redetermined every X seconds.")

local gd_npc_rainbow_chase_repath_interval =
	CreateConVar("gd_npc_rainbow_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of rainbow's target will be redetermined every \z
	X seconds.")

local gd_npc_rainbow_expensive_scan_interval =
	CreateConVar("gd_npc_rainbow_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local gd_npc_rainbow_force_download =
	CreateConVar("gd_npc_rainbow_force_download", 1, FCVAR_ARCHIVE,
...
util.AddNetworkString("rainbow_nag")
util.AddNetworkString("rainbow_navgen")
...
local VECTOR_rainbow_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_rainbow_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("gd_npc_rainbow: found %d suitable (%d unsuitable) hiding \z
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("gd_npc_rainbow: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("gd_npc_rainbow: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("gd_npc_rainbow: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("gd_npc_rainbow: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("gd_npc_rainbow: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, rainbow in pairs(ents.FindByClass("gd_npc_rainbow")) do
		rainbow:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		print("gd_npc_rainbow: nav_generate failed to initialize")
...
concommand.Add("gd_npc_rainbow_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("gd_npc_rainbow: Beginning nav_generate requested by " .. ply:Name())
	...
	elseif isConsole then
		print("gd_npc_rainbow: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = gd_npc_rainbow_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `gd_npc_rainbow_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if gd_npc_rainbow_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = gd_npc_rainbow_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. rainbow means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not gd_npc_rainbow_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = gd_npc_rainbow_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = gd_npc_rainbow_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = gd_npc_rainbow_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = gd_npc_rainbow_attack_distance:GetInt()
		...
		local repathInterval = gd_npc_rainbow_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and gd_npc_rainbow_allow_jump:GetBool()
		...
		local hidingScanInterval = gd_npc_rainbow_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = gd_npc_rainbow_hiding_repath_interval:GetFloat()
...
local MAT_rainbow = Material("nextbot/gd_npc_rainbow/rainbow")
killicon.Add("gd_npc_rainbow", "nextbot/gd_npc_rainbow/rainbow", color_white)
language.Add("gd_npc_rainbow", "rainbow ")
...
local gd_npc_rainbow_music_volume =
	CreateConVar("gd_npc_rainbow_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by rainbow. (0-1, where 0 is muted)")

-- If another rainbow comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, rainbows do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_rainbow_PANIC_COUNT rainbows are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of rainbow.
local MUSIC_rainbow_PANIC_COUNT = 8

local MUSIC_rainbow_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_rainbow_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("gd_npc_rainbow") == 0 then
	...
	local userVolume = math.Clamp(gd_npc_rainbow_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "gd_npc_rainbow" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_rainbow_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear rainbow taunt us.
...
local function startTimer()
	if not timer.Exists("rainbowPanicMusicUpdate") then
		timer.Create("rainbowPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_rainbow)

	-- Get the normal vector from rainbow to the player's eyes, and then compute
	...
	-- rainbow should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("rainbowHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("rainbowHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("rainbow is studying this map.", "rainbowHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "rainbowHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "rainbowHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "rainbowHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "rainbowHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("rainbow_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "rainbowNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. rainbow doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("gd_npc_rainbow_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for rainbow to figure \z
		this map out.\n\z
...
net.Receive("rainbow_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! rainbow doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet rainbow-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! rainbow doesn't know this map. \z
			...
			\n\z
			If you ARE the server host, you can run gd_npc_rainbow_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not rainbow-compatible!",
			"Ok", nil,
...
list.Set( "NPC", "gd_npc_rainbow", {
	Name = "rainbowdoesn'tstuff", 
	Class = "gd_npc_rainbow", 
	Category = "Nextbot",
	AdminOnly = false,
})
